...
5. Leave image and vision model overrides empty for dynamic discovery.
6. Open Settings → MCP and confirm the namespaced bundled `oxoxos-cocos-game-assets` server is running. The first start uses `uv` to create the isolated environment in the plugin cache and may take longer.
7. Start a fresh ZCode session so the MCP tools and installation Skill are injected.
8. Call `list_models` before generation.
